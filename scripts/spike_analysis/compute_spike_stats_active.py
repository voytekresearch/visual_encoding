...
def main():
    ...
    for i_file, fname in enumerate(files):
        ...
        for i_seg, segment in enumerate(block.segments):
            ...
            for structure in brain_structures:
                ...
                # filter for active units
                spiketrains = neo.core.spiketrainlist.SpikeTrainList([spiketrain for spiketrain in spiketrains 
                                                                      if spiketrain.annotations['unit_id'] in active_units])
    ...
    df_region.to_csv(f'{dir_output}/region_metrics/{STIM_CODE}_filtered.csv', index=False)
    ...
    df_units.to_csv(f'{dir_output}/unit_rates/{STIM_CODE}_filtered.csv', index=False)
