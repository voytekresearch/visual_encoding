...
threshold crossings. find_segments() and the underlying functions are 
generalized to find segments above or below a threshold in any signal.

For this project, these functions are used in to split each session 
into 'running' and 'stationary' epochs based on the velocity data from the
running wheel (in scripts/pipeline/step2_create_spontaneous_epochs.py). 
